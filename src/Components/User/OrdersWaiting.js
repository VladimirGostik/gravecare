...
const OrdersWaiting = () => {
  const { fetchByCustomerNotConfirmedId } = useOrders();  // Použijeme funkciu na získanie nevybavených objednávok
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      if (user?.id) {
        try {
          const orders = await fetchByCustomerNotConfirmedId(user.id); // Získavame nevybavené objednávky pre daného používateľa
    ...
    fetchOrders();
  }, [fetchByCustomerNotConfirmedId, user]);
  ...
  return (
    <>
      <div className="text-left mb-4 ml-4">
        <h2 className="text-2xl font-bold text-white">Objednávky čakajúce na potvrdenie:</h2>
      </div>
