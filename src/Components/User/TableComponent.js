...
import TableItem from './TableItem'; // Importujeme nový komponent pre položky tabuľky
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col m-4">
      ...
      <div className="flex justify-between items-center text-white font-bold pr-2">
        ...
        >
          Dátum {sortConfig.key === 'deadline' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
        </span>
      ...
      <div>
        {currentOrders.length > 0 ? (
          currentOrders.map((order) => (
            <TableItem key={order.id} order={order} onOrderClick={onOrderClick} />
          ))
        ) : (
