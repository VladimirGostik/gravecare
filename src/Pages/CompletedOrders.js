...
import CompletedOrdersItem from '../Components/CompletedOrdersItem'; // Nový komponent
import { useOrders } from '../Context/OrderContext'; // Importujeme OrderContext

const CompletedOrders = () => {
    const { fetchCompletedOrders } = useOrders();  // Použijeme funkciu na získanie nevybavených objednávok
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const orders = await fetchCompletedOrders(); // Získavame nevybavené objednávky
                ...
                <div>
                    {currentOrders.length > 0 ? (
                        currentOrders.map((order) => (
                            <CompletedOrdersItem key={order.id} order={order} />
                        ))
                    ) : (
